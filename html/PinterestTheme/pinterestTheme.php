...
$sql = "SELECT * FROM image INNER JOIN book ON book.isbn = image.isbn\n"
    . "WHERE `year` > 2012\n"
    ...
    <script>
      $(document).ready(function(){
        $('li a').click(function() { 
          ...
          $.get('modal.php', { isbn: id }, function(data){
            $('#myModal').append(data);
          });
        });
      });
      ...
      <div class="images">
        <ul class="grid effect-1" id="grid">
          ...
          while($book = $books->fetch_assoc()){
            if($book['med_img']){
              print "<li><a href='#' class='big-link' data-reveal-id='myModal' data-animation='fade' data-id='".$book['isbn']."'><img src='".$book['med_img']."' alt=' ".$book['title']."' class='my_box'></a></li>";
            } elseif ($book['large_img']) {
              print "<li><a href='modal.php' class='big-link' data-reveal-id='myModal' data-animation='fade' data-id='".$book['isbn']."'><img src='".$book['large_img']."' alt=' ".$book['title']."' class='my_box'></a></li>";
            }
          }
          ?>
        </ul>
